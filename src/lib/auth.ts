...
async function handleTwitterSignIn(supabase: ReturnType<typeof getSupabase>, profile: any): Promise<boolean> {
  ...
  if (existingUser) {
    ...
    await createNewUser(supabase, { ...userData, credit_balance: NEW_USER_BONUS_CREDITS }, 'twitter_id', twitterId);
  }
  return true;
}
...
async function handleGoogleSignIn(supabase: ReturnType<typeof getSupabase>, profile: any): Promise<boolean> {
  ...
  if (existingUser) {
    ...
    await createNewUser(supabase, { ...userData, credit_balance: NEW_USER_BONUS_CREDITS }, 'google_id', googleId);
  }
  return true;
}
