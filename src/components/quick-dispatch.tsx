    ...
    return (
      <section className="container mx-auto px-4 py-20">
        <div className="max-w-5xl mx-auto">
          <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: 'rgb(var(--t-parchment) / 0.35)', fontFamily: 'var(--font-oswald)' }}>Quick Synthesis</p>
          <p className="text-sm" style={{ color: 'rgb(var(--t-parchment) / 0.4)' }}>Loading primary analysts…</p>
        </div>
      </section>
    );
  }
  ...
  if (sources.length === 0) {
    return (
      <section className="container mx-auto px-4 py-20">
        <div className="max-w-5xl mx-auto">
          <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: 'rgb(var(--t-parchment) / 0.35)', fontFamily: 'var(--font-oswald)' }}>Quick Synthesis</p>
          <p className="text-sm" style={{ color: 'rgb(var(--t-parchment) / 0.4)' }}>Primary analysts coming soon.</p>
        </div>
      </section>
    );
  }
    ...
    <section className="container mx-auto px-4 py-20">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-end justify-between mb-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: 'rgb(var(--t-parchment) / 0.35)', fontFamily: 'var(--font-oswald)' }}>
              Quick Dispatch
            </p>
            <h2 className="text-3xl font-bold">Quick Synthesis</h2>
          </div>
          <span className="text-xs hidden md:block" style={{ color: 'rgb(var(--t-parchment) / 0.4)', fontFamily: 'var(--font-mono)' }}>
            Pick up to {MAX_SELECT} · 1 free run/day
          </span>
        </div>
        <p className="text-sm mb-8 max-w-2xl" style={{ color: 'rgb(var(--t-parchment) / 0.5)' }}>
        ...
        <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: 'rgb(var(--t-parchment) / 0.4)', fontFamily: 'var(--font-oswald)' }}>
          Featured analysts
        </p>
          ...
          {sources.map((s) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isSelected ? 'rgb(var(--t-brass) / 0.12)' : 'transparent',
                  border: isSelected ? '1px solid rgb(var(--t-brass))' : '1px solid rgb(var(--t-brass) / 0.18)',
                }}
              >
                <div className="flex items-center gap-3 mb-2">
                  {s.avatar_url ? (
                    ...
                    <div className="w-8 h-8 rounded-sm flex items-center justify-center" style={{ background: 'rgb(var(--t-raised))' }}>
                      <span className="text-xs" style={{ color: 'rgb(var(--t-brass))' }}>@</span>
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold truncate" style={{ color: 'rgb(var(--t-parchment))' }}>
                      {s.display_name || `@${s.handle}`}
                    </div>
                    <div className="text-xs truncate" style={{ color: 'rgb(var(--t-parchment) / 0.4)', fontFamily: 'var(--font-mono)' }}>
                      @{s.handle}
                    </div>
                  </div>
                  {isSelected && (
                    <span className="text-xs px-2 py-0.5 rounded-sm font-medium" style={{ background: 'rgb(var(--t-brass))', color: 'rgb(var(--t-ink))', fontFamily: 'var(--font-oswald)' }}>
                      ✓
                    </span>
                  )}
                </div>
                {s.bio && (
                  <p className="text-xs leading-relaxed" style={{ color: 'rgb(var(--t-parchment) / 0.45)' }}>
              ...
              {trackedOnly.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[10px] uppercase tracking-wider" style={{ color: 'rgb(var(--t-brass) / 0.55)', fontFamily: 'var(--font-oswald)' }}>
                      from {s.junto_name}
                    </p>
                  </button>
                );
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            style={{
              background: buttonDisabled ? 'rgb(var(--t-brass) / 0.25)' : 'rgb(var(--t-brass))',
              color: 'rgb(var(--t-ink))',
          ...
          {isAuthed && !usedToday && !result && !running && (
            <span className="text-xs" style={{ color: 'rgb(var(--t-parchment) / 0.4)', fontFamily: 'var(--font-mono)' }}>
              Costs 5 credits
            </span>
          )}
        </div>
        ...
        {/* Output */}
        {result && (
          <div className="mt-8 p-6 rounded-sm" style={{ border: '1px solid rgb(var(--t-brass) / 0.28)', background: 'rgba(20,18,16,0.6)' }}>
            <div className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: 'rgb(var(--t-brass) / 0.8)', fontFamily: 'var(--font-oswald)' }}>
              Dispatch
            </div>
            <h3 className="text-2xl font-bold mb-4 leading-tight">{result.subject}</h3>

            <div className="prose prose-invert max-w-none text-sm leading-relaxed whitespace-pre-wrap" style={{ color: 'rgb(var(--t-parchment) / 0.85)' }}>
              {result.content}
            </div>

            {result.positions.length > 0 && (
              <div className="mt-6 pt-6" style={{ borderTop: '1px solid rgb(var(--t-brass) / 0.18)' }}>
                <div className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: 'rgb(var(--t-brass) / 0.8)', fontFamily: 'var(--font-oswald)' }}>
                  Positions they agree on
                </div>
                <ul className="space-y-2">
                  {result.positions.map((p) => (
                    <li key={`${p.ticker}-${p.stance}`} className="flex items-baseline gap-3 text-sm">
                      <span className="font-semibold" style={{ color: 'rgb(var(--t-parchment))', fontFamily: 'var(--font-mono)' }}>${p.ticker}</span>
                      <span
                        ...
                        style={{
                          ...
                          color: p.stance === 'bullish' ? 'rgb(var(--t-bull))' : '#ff8888',
                          fontFamily: 'var(--font-oswald)',
                        }}
                      >
                        {p.stance}
                      </span>
                      <span style={{ color: 'rgb(var(--t-parchment) / 0.55)' }}>
            ...
            {result.sourcesUsed.some((s) => !s.used) && (
              <p className="text-xs mt-4" style={{ color: 'rgb(var(--t-parchment) / 0.35)' }}>
            ...
            <div className="mt-6 flex items-center gap-3">
              <button
                ...
                style={{ border: '1px solid rgb(var(--t-brass) / 0.35)', color: emailSent ? 'rgb(var(--t-bull))' : 'rgb(var(--t-parchment) / 0.7)' }}
              >
