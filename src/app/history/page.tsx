  ...
  return (
    <main className="min-h-screen bg-ink text-parchment">
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <p className="text-parchment/60 mb-8">All generated issues from your subscriptions and dispatches.</p>
            ...
            {runs.map((run) => (
              <div key={run.id} className="bg-surface border border-[rgb(var(--t-brass) / 0.28)] rounded overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between p-5 hover:bg-raised transition text-left"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="font-semibold text-parchment truncate">
                        {run.subject || 'Untitled issue'}
                      </h3>
                      {run.newsletter_name && (
                        <span className="text-xs px-2 py-0.5 rounded-sm bg-brass/15 text-brass border border-[rgb(var(--t-brass) / 0.3)] font-medium shrink-0">
                          {run.newsletter_name}
                        </span>
                      )}
                    </div>
                ...
                {expandedId === run.id && (
                  <div className="border-t border-[rgb(var(--t-brass) / 0.18)] p-6">
                    <div
                      className="prose prose-invert prose-sm max-w-none text-parchment/80 leading-relaxed"
